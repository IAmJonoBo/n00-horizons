...
1. **Generate slices** – run `pnpm run handover:auto` (or CI workflow) so `scripts/slicing/slice_plan.py` ingests `Next_Steps*.md`, Horizons docs, and `n00tropic_HQ/98. Internal-Projects/**`.
...
4. **Resolve** – update the canonical doc(s), run `project.capture` for each changed file, then rerun `pnpm run handover:auto` to clear the flags.

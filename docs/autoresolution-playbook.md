...
| Metadata conflicts             | `pnpm run handover:auto` (invokes `scripts/slicing/slice_plan.py`)                                | `n00-cortex/artifacts/slices.enriched.json`                   | Flags duplicate IDs, tag drift, and inconsistent owners/status across sources.  |
